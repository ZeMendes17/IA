...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        ...
        # action -> Operator
        # 1. verificar precondicoes
        if not all([pre in state for pre in action.pc]): # all preconditions are in state
            return None
         
        # 2. efeito negativo
        newstate = [s for s in state if s not in action.neg] # remove negative effects
    
        # 3. efeito positivo
        return set(newstate + action.pos) # add positive effects
